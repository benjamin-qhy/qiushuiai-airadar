# AI Radar 运行架构与完整处理流程

> 本文依据 2026-09-16 的当前代码编写，描述的是“现在实际怎样运行”，不是未来设想。
>
> 一句话理解：AI Radar 像一条内容加工流水线——先去各个平台找新内容，再补齐正文、字幕或图文附件，最后交给 AI 做摘要和评分，合格内容才进入每日内容流。
...
flowchart LR
    ...
    M[AI 分析器<br/>摘要、主题、七项评分、垃圾判断<br/>英文 X 普通帖另附中文意译]
    R[(正式 Markdown 档案)]
...
flowchart TD
    ...
    FP -- 否 --> AI[AI 一次生成<br/>中文摘要、主题、七项1-5档评分、垃圾判断<br/>英文 X 普通帖另附中文意译]
    AI --> VALID{结果格式完整有效?}
    VALID -- 否 --> TASKFAIL
...
- 指纹不同：调用当前固定模型 `gpt-5.3-codex-spark`。

模型一次返回中文摘要、主题、七项 1～5 档评分以及垃圾判断。对于**原文为英文的 X 普通帖**，这同一次调用还会返回完整中文意译；X 长文和其他英文内容目前不在这一翻译范围内。需要意译却没有返回意译时，本次分析算失败。格式不完整也算失败，不会用残缺结果充数。
...
- 完整性门禁：材料不完整不能分析。
- AI 摘要、主题、七项评分、垃圾判断和推荐分层；英文 X 普通帖另有中文意译。
...
5. **自动转写依赖外部服务。** 除已接入的抖音 Get笔记路径外，其他无字幕视频需要配置 `AIRADAR_TRANSCRIBER_URL`，否则会等待人工处理或失败。
6. **分析模型路线目前写死。** 正式服务固定使用 `gpt-5.3-codex-spark`；配置页可以调整画像和评分，但不能切换模型路线。
...
| `originalLanguage` | 原文语言：`zh` 中文、`en` 英文、`unknown` 暂无法可靠判断；依据正文自动识别。 |
| `chineseTranslation?` | 完整中文意译；目前只为英文 X 普通帖生成。它与摘要分开保存，列表优先显示它。 |
...
| `source?` | 来自哪个信源；里面有 `id` 信源编号、`name` 显示名称、`type` 平台类型。 |
...
页面列表对英文 X 普通帖优先显示 `chineseTranslation`，详情仍可查看 `body` 英文原文；没有意译的旧帖会标“原文英文·未意译”，可以人工触发生成。中文、英文之外或正文尚为空时，语言可显示“待识别”，不会猜成中文或英文。
...
默认根目录是 `~/.airadar/data/`；若启动时指定 `AIRADAR_DATA_ROOT`，则以指定目录为准。下面用假设的内容 ID `x:123456` 举例。`<SHA-256(...)>` 表示按括号中的完整 ID 计算出的文件名，不是字面文件名。不同记录的 ID 不同，因此哈希文件名也不同。
