...
4. **语音转写并未自动开启。** [后台配置](../../apps/service/src/index.ts)允许通过 `QIUSHUIAI_AIRADAR_TRANSCRIBER_URL` 接入转写服务，抖音另有 Get笔记转写路径；但本机验收数据的任务参数里 `transcription.auto=false`。即使取得视频 URL，也还需要可用的转写服务、有效媒体文件和实际任务调度，才能生成逐字正文。
